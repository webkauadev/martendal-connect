...
def ready(page):
    ...
    if (
        urlparse(page.url).path.endswith(("/machos", "/femeas"))
...
def whatsapp(href, key, animal=None, number=None):
    ...
    catalog = "catálogo Fêmeas Elite do" if key == "femeas" else "catálogo do"
    expected = (
    ...
    for width in [320, 375, 390, 768, 1440]:
        ...
        for key, catalog_name in NAMES.items():
            ...
            first = (
                ...
                if key == "femeas"
                else "SPOOKS GOTTA SHINE"
            )
            owner = "PECUARIA MARTENDAL" if key == "femeas" else "MARCUS FERRARI"
            ...
            goto_lot(page, "01")
            lot = page.locator("#lote-01")
            whatsapp(
                lot.locator(".cat-interest").get_attribute("href"), key, first, "01"
            )
            whatsapp(page.locator(".cat-cta").get_attribute("href"), key, first, "01")
            ...
            first_number = "01" if key == "femeas" else "-100"
            page.wait_for_function(
        ...
        print(
            f"PASS {width}px: selector, both catalogs, searches, scroll, CTAs, videos, UTM, Pixel, internal tracking, session dedup, no overflow",
            flush=True,
        )
        context.close()
